...
// expose Location response header for client
// used in create course POST
...
router.get('/:id', asyncHandler(async (req, res) => {
    ...
    if (course) {
        ...
        res.status(404).json({message: `Course of id ${req.params.id} was not found`}).end();
    }
}));

// POST /api/courses 201 - Creates a course, sets the Location header to the URI for the course, and returns no content
router.post('/', authenticateUser, cors(corsOptions), asyncHandler(async (req, res) => {
...
], authenticateUser, cors(corsOptions), asyncHandler(async (req, res) => {
    ...
    if (!errors.isEmpty()) {
        ...
        if (course) {
            if (course.userId === req.currentUser.id) {
                ...
                res.status(204).location(`/${course.id}`).end();
            } else {
